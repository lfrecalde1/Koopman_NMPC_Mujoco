...
def lift_Fun_angular(x, cent_a):
    x_lift = []
    for k in x: x_lift.append(k)
    x_lift.append(np.sin(x[0])*np.tan(x[1]))
    x_lift.append(np.cos(x[0])*np.tan(x[1]))
    x_lift.append(np.cos(x[0]))
    x_lift.append(np.sin(x[0]))
    x_lift.append(np.sin(x[0])/np.cos(x[1]))
    x_lift.append(np.cos(x[0])/np.cos(x[1]))
    x_lift.append(x[3]*x[4])
    x_lift.append(x[3]*x[5])
    x_lift.append(x[4]*x[5])
    x_lift.append(np.cos(x[0])*np.sin(x[1])*np.cos(x[2]) + np.sin(x[0])*np.sin(x[2]))
    x_lift.append(np.cos(x[0])*np.sin(x[1])*np.sin(x[2]) - np.sin(x[0])*np.cos(x[2]))
    x_lift.append(np.cos(x[0])*np.cos(x[1]))
    x_lift = np.array(x_lift)
    return x_lift
